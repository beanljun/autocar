...
const double ROBOT_LENGTH = 137.00;  //mm
const double ROBOT_RADIUS = 68.50;  //两轮之间的半径长度mm
...
void writeSpeed(double RobotV, double YawRate,unsigned char ctrlFlag)
{
    ...
    try {
        // 通过串口下发数据
        std::size_t bytes_transferred = boost::asio::write(sp, boost::asio::buffer(buf));
        // std::cout << "数据发送成功，发送字节数：" << bytes_transferred << std::endl;
    } catch (boost::system::system_error& e) {
        std::cerr << "数据发送失败，失败原因：" << e.what() << std::endl;
    }
...
出口参数：bool
********************************************************/
bool readSpeed(double &vx,double &vth,unsigned char &ctrlFlag)
{
    char i, length = 0;
